...
export function extractDevAddressV6(
  input: Record<string, string>,
): IDevAddressV6 {
  const ipV6Addresses = Object.keys(input)
    .filter((key) => key.startsWith("IP6.ADDRESS"))
    .map((key) => input[key]);
  //   console.log({ ipV6Addresses });
  const notLocalLink = ipV6Addresses.filter(
    (address) => !address.startsWith("fe80::"),
  );
  //   console.log({ notLocalLink });

  // console.log({ input });
  // console.log(input['GENERAL.TYPE']);
  const { ipAddress, subnetPrefix } = extractIPInfoV6(notLocalLink[0]);
  //   console.log({ ipAddress, subnetPrefix });
...
export async function getDeviceStats(
  connectObjs: NetworkInfo[],
): Promise<IDevAddress[]> {
  const stats = await Promise.all(
    connectObjs.map((con) => getDeviceStat(con.device)),
  );
  // console.log(stats.length);
  const records: Record<string, string>[] = stats.map((stat) =>
    parseDeviceStat(stat),
  );
  ...
  const devAddresses: IDevAddress[] = records.map((record) =>
    extractDevAddress(record),
  );
...
export async function getDeviceV6Stats(
  connectObjs: NetworkInfo[],
): Promise<IDevAddressV6[]> {
  const stats = await Promise.all(
    connectObjs.map((con) => getDeviceStat(con.device)),
  );
  // console.log(stats.length);
  ...
  const devAddresses: IDevAddressV6[] = records.map((record) =>
    extractDevAddressV6(record),
  );
  // console.log(devAddresses)
  // return devAddresses.filter(dev => dev.address !== null);
  return devAddresses;
